namespace AudioUI
{
    ...
    /// 收集畫面繫結失效的訊息。
    ///
    /// 這個專案從分家起唯一驗不到的失效模式就是繫結錯誤——它不會讓編譯失敗、不會讓測試變紅，
    /// 執行時只是那一格空白，看起來跟「本來就沒東西」一模一樣。這裡把它變成數得出來的東西。
    ///
    /// 刻意去重：一條壞掉的繫結寫在 DataTemplate 裡，畫面上有幾筆資料就會重複報幾次，
    /// 五十筆清單能把一個錯誤放大成五十行，真正的第二個錯誤就被推出視線外了。
    /// </summary>
